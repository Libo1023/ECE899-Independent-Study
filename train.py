    ...
    for epoch in range(0, epochs) :
        ...
        for phase in ["train", "validation"] :
            ...
            if phase == "train" :
                ...
                if epoch >= cut_epochs :
                    # Introduce the transformer after 50 epochs to avoid overfitting
                    model_transformer = model_transformer.train(True)
                else :
                    # How to fix the output dimension issue?
                    # Introduce an upscaling linear transformation
                    # Replace FC1 in the classifier with the transformer
            ...
            for each_idx in random_idx :
                ...
                # Level-1 Sampled Map (with Replacement)
                ...
                for x in s1_tuples :
                    ...
                    if pad_flag_X == 0 :
                        # Only pad one whole slide image one time
                        pad_flag_X = 1
                        # Pad the whole slide image to avoid slicing out of bounds
                        X = np.pad(X, pad_width = ((0, h1), (0, w1), (0, 0)),
                                   mode = "constant", constant_values = 0)
                    # Slice sampled tiles from the whole slide image
                    ...
                    # Introduce entropy regularization loss towards level-2 attention
                    loss_beta = loss_beta + x[1] * er_beta(out_a_beta)
                    # Level-2 Sampled Map (without Replacement)
                    ...
                    for s2_x in s2_samples :
                        ...
                        # print((i, j), (s2_i, s2_j))
                        if pad_flag_Xa == 0 :
                            # Only pad one sampled tile one time
                            pad_flag_Xa = 1
                            # Pad the sampled tile to avoid slicing out of bounds
                            Xa = np.pad(Xa, pad_width = ((0, h2), (0, w2), (0, 0)),
                                        mode = "constant", constant_values = 0)
                        # Slice sampled sub-tiles from the sampled tile
                        Xb = np.copy(Xa[int(s2_i/s2):int(s2_i/s2+h2),
                                        int(s2_j/s2):int(s2_j/s2+w2), :])
                        Xb = np.reshape(Xb, (3, Xb.shape[0], Xb.shape[1]))
                        # Concatenate all sampled sub-tiles from different tiles
                ...
                if epoch < cut_epochs :
                    ...
                    # Visualize some self-attention weights
                    if each_idx < 2 : print(Q_K[0:2])
                #################################################################
                if torch.equal(torch.round(y_pred_ds), Y) :
                    num_correct = num_correct + 1
                print("y = %.0f" % Y[0], "y_pred = %.3f" % y_pred_ds[0])
                loss = criterion(y_pred_ds, Y)
                # Introduce entropy regularization loss towards level-1 attention
                loss_alpha = er_alpha(out_alpha)
                loss = loss + loss_alpha + loss_beta
                ...
                if phase == "train" :
                    if epoch >= cut_epochs :
                        ...
                        # Utilize optimizer1 to optimize the baseline
                        # Introduce optimizer2 to optimize the transformer
                        ...
                        # Do not introduce/optimize the transformer
                        # Utilize optimizer1 to optimize the baseline
                        optimizer1.zero_grad()
                        loss.backward()
                        optimizer1.step()
            ############################################################
            # Obtain decision statistics to prepare for the ROC plot
            if phase == "train" :
